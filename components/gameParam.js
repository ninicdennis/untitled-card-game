...
const drawCard = (G, ctx) => {
  ...
  if (G[ctx.currentPlayer].deck.length !== 0) {
    const drawNum = Math.floor(
      Math.random() * G[ctx.currentPlayer].deck.length,
    );
...
export const GameParam = {
  ...
  isActive: true,
  turn: {
    // Stages must be set thru a useEffect checking who the active player is. A stipulation since phases get removed after they end.
